...
TEMPDIR=`mktemp -d /tmp/xrdclient.srpm.XXXXXXXXXX`
...
git archive --prefix=xrdclient/ --format=tar $COMMIT | gzip -9fn > \
      $RPMSOURCES/xrdclient.tar.gz
...
#-------------------------------------------------------------------------------
if test ! -r rhel/xrdclient.spec.in; then
  echo "[!] The specfile template does not exist!" 1>&2
  exit 7
fi
cat rhel/xrdclient.spec.in | sed "s/__VERSION__/$VERSION/" | \
  sed "s/__RELEASE__/$RELEASE/" > $TEMPDIR/xrdclient.spec
...
rpmbuild --define "_topdir $TEMPDIR/rpmbuild"    \
  ...
  -bs $TEMPDIR/xrdclient.spec > $TEMPDIR/log
if test $? -ne 0; then
  echo "[!] RPM creation failed" 1>&2
  exit 8
fi

cp $TEMPDIR/rpmbuild/SRPMS/xrdclient*.src.rpm $OUTPUTPATH
rm -rf $TEMPDIR
